...
class SaleOrder(generic.BO):
    ...
    user = models.ForeignKey(User,verbose_name=_("sales man"),blank=True,null=True,on_delete=models.SET_NULL)
...
class SaleItem(models.Model):
    ...
    material = models.ForeignKey(Material,verbose_name=_("material"),limit_choices_to={"is_virtual":"0",'can_sale':'1'},blank=True,null=True,on_delete=models.SET_NULL)
...
class PaymentCollection(generic.BO):
    ...
    bank = models.ForeignKey(BankAccount,verbose_name=_("bank account"),blank=True,null=True,on_delete=models.SET_NULL)
...
class OfferSheet(generic.BO):
    ...
    amount = models.DecimalField(_("money amount"),max_digits=12,decimal_places=2,blank=True,null=True,default=0.00)
    discount_amount = models.DecimalField(_("discount amount"),max_digits=12,decimal_places=2,blank=True,null=True,default=0.00)
    user = models.ForeignKey(User,verbose_name=_("offer man"),blank=True,null=True,on_delete=models.SET_NULL)
            ...
            with transaction.atomic():
                ...
                for row_index in range(row_count):
                    ...
                    try:
                        ...
                        measure = Measure.objects.create(code=row[4],name=force_str(row[5]))

                    try:
                        material = Material.objects.get(code=row[0])
                    except Exception as e:
                        material = Material(code=row[0],name=force_str(row[1]),spec=force_str(row[2]))
                        material.sale_price = row[6]
                        material.purchase_price = row[7]
                        material.save()
                    OfferItem.objects.create(master=self,material=material,measure=measure,cnt=row[8],brand=force_str(row[3]),
