...
class Site(models.Model):
    """
    Sites. There can be multiple companies under one site. Users under the same site are logically in the same organization.
    """
    index_weight = 1
...
class Module(generic.BO):
    ...
    parent = models.ForeignKey('self',blank=True,null=True,verbose_name=_("parent"),on_delete=models.SET_NULL)
...
class Role(generic.BO):
    """
    Role management, assigning menus to users
    """
    index_weight = 4
